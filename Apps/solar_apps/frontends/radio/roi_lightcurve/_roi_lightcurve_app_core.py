"""Private core implementation for roi_lightcurve_app.py.

The public facade lives in the parent module; monkeypatchable names are
read through the facade at call time to preserve test patching.
"""
...
import solar_apps.frontends.radio.roi_lightcurve.roi_lightcurve_app as _facade  # noqa: E402
...
def build_reference_figure(
    ...
    *,
    ...
    return _facade._build_reference_figure_from_preview(
        item,
        preview,
        roi=roi,
...
def _render_load_step(
    ...
    with c1:
        radio_dir = _facade.render_native_path_input(
            st,
            "Radio FITS folder",
            key="radio_dir",
    ...
    output_dir = _facade.render_native_path_input(
        st,
        "Output folder",
        key="output_dir",
...
def _render_roi_import_controls(
    ...
    roi_json_path = _facade.render_native_path_input(
        st,
        "ROI JSON path",
        key="roi_json_path",
        initial_value="",
...
def _render_roi_step(
    ...
    _facade._render_roi_import_controls(st, path_policy, ui_store)
    ...
    for index, (reference, preview) in enumerate(
        ...
        with columns[index % len(columns)]:
            ...
            figure = _facade._build_reference_figure_from_preview(
                reference,
                preview,
                roi=active_roi,
                roi_mode=roi_mode,
...
def _render_analysis_and_export_steps(
    ...
    df = _facade._render_analysis_step(
        st,
        selected_paths,
        references,
        roi,
