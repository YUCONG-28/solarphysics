...
PRODUCT_FILENAMES = {
    ...
    "coordinates_csv": "radio_roi_coordinates.csv",
...
_LIGHTCURVE_PLOT_STYLES = ("scatter", "line")
_LIGHTCURVE_Y_TRANSFORMS = ("linear", "log10")
_ANGULAR_UNITS = {
    "arcsec",
    "arcsecs",
    "arcsecond",
...
def _roi_coordinates_frame(roi: RadioRoi) -> pd.DataFrame:
    """Return a closed HPLN/HPLT vertex sequence for later image annotation."""

    vertices = [(float(x), float(y)) for x, y in roi.vertices_arcsec]
    if not vertices:
        raise ValueError("ROI has no vertices to export")
    closed_vertices = [*vertices, vertices[0]]
    bounds = roi.bounds_arcsec
    rows = []
    for order, (x_arcsec, y_arcsec) in enumerate(closed_vertices):
        rows.append(
            {
                "roi_id": roi.roi_id,
                "roi_kind": roi.kind,
                "coordinate_system": "HPLN/HPLT arcsec",
                "vertex_order": order,
                "hpln_arcsec": x_arcsec,
                "hplt_arcsec": y_arcsec,
                "is_closure_vertex": order == len(vertices),
                "left_arcsec": float(bounds["left"]),
                "bottom_arcsec": float(bounds["bottom"]),
                "right_arcsec": float(bounds["right"]),
                "top_arcsec": float(bounds["top"]),
            }
        )
...
def write_radio_roi_products(
    ...
    *,
    ...
    artifacts = build_radio_roi_artifacts(
        ...
        lightcurve_plot_style=lightcurve_plot_style,
        lightcurve_y_transform=lightcurve_y_transform,
        selected_products=product_keys,
...
def build_radio_roi_artifacts(
    ...
    *,
    ...
    if "coordinates_csv" in product_keys:
        artifacts["coordinates_csv"] = (
            _roi_coordinates_frame(roi).to_csv(index=False).encode("utf-8-sig")
        )
    if "json" in product_keys:
        ...
        settings["lightcurve_plot"] = _lightcurve_plot_settings(
            ...
            metric_unit=_metric_unit_label(df, metric),
        )
        selection = {
            "schema_version": ROI_SCHEMA_VERSION,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--plot-style",
        choices=list(_LIGHTCURVE_PLOT_STYLES),
        default="scatter",
        help="Render exported light curves as scatter points or connected lines.",
    )
    parser.add_argument(
        "--y-transform",
        choices=list(_LIGHTCURVE_Y_TRANSFORMS),
        default="linear",
        help="Use raw values or log10 of positive values in exported plots.",
    )
    parser.add_argument(
...
def run_radio_roi_lightcurve(argv: list[str] | None = None) -> dict[str, Path]:
    ...
    return write_radio_roi_products(
        ...
        lightcurve_plot_style=args.plot_style,
        lightcurve_y_transform=args.y_transform,
...
def main(argv: list[str] | None = None) -> int:
    ...
    labels = {
        "csv": "Statistics CSV",
        "coordinates_csv": "ROI coordinates CSV",
        "json": "ROI JSON",
...
def _plot_radio_roi_lightcurve(
    ...
    *,
    ...
    """Render one independently scaled panel per frequency."""
    ...
    normalized_plot_style = _normalize_lightcurve_plot_style(plot_style)
    normalized_y_transform = _normalize_lightcurve_y_transform(y_transform)
    path.parent.mkdir(parents=True, exist_ok=True)
    data = _lightcurve_plot_data(
        df,
        metric,
        y_transform=normalized_y_transform,
    )
    ...
    for index, axis in enumerate(axes):
        if index % min(4, max(1, len(frequencies))) == 0:
            axis.set_ylabel(_metric_axis_label(df, metric, normalized_y_transform))
    if frequencies:
        column_count = min(4, len(frequencies))
...
def _plot_radio_roi_lightcurve_detail(
    ...
    *,
    ...
    """Render one selected frequency as a large light curve."""
        ...
        for polarization, group in frequency_data.groupby(
            ...
            if np.any(outside):
                axis.scatter(
                    ...
                    label=(
                        label
                        if normalized_plot_style == "scatter" and not np.any(~outside)
                        else "_nolegend_"
                    ),
                )
    if not frequencies:
...
def _lightcurve_plot_data(
    df: pd.DataFrame,
    metric: str,
    *,
    y_transform: str = "linear",
) -> pd.DataFrame:
    normalized_transform = _normalize_lightcurve_y_transform(y_transform)
    data = df.copy()
    ...
    if normalized_transform == "log10":
        valid &= metric_values > 0.0
...
def _draw_lightcurve_frequency(
    axis: Any,
    data: pd.DataFrame,
    *,
    metric: str,
    ...
    normalized_style = _normalize_lightcurve_plot_style(plot_style)
    for polarization, group in data.groupby("polarization", dropna=False, sort=True):
        ordered = group.sort_values("obs_time_dt")
        marker = _lightcurve_polarization_marker(polarization)
        if normalized_style == "line":
            axis.plot(
                ordered["obs_time_dt"],
                ordered[metric],
                color=color,
...
def _normalize_lightcurve_plot_style(value: str) -> str:
    normalized = str(value).strip().lower()
    if normalized not in _LIGHTCURVE_PLOT_STYLES:
        choices = ", ".join(_LIGHTCURVE_PLOT_STYLES)
        raise ValueError(f"lightcurve_plot_style must be one of: {choices}")
    return normalized


def _normalize_lightcurve_y_transform(value: str) -> str:
    normalized = str(value).strip().lower()
    aliases = {
        "log": "log10",
    ...
    if normalized not in _LIGHTCURVE_Y_TRANSFORMS:
        choices = ", ".join(_LIGHTCURVE_Y_TRANSFORMS)
        raise ValueError(f"lightcurve_y_transform must be one of: {choices}")
    return normalized
...
def _lightcurve_plot_settings(
    *,
    ...
    return {
        ...
        "normalization": (
            "linear within each frequency display range after the selected "
            "Y transform; clipped to [0, 1]"
        ),
    }
...
def _metric_unit_label(df: pd.DataFrame, metric: str) -> str:
    ...
    base_unit = bunit or "FITS unit"
    if metric == "raw_sum":
        return f"{base_unit} * pixel"
    return base_unit


def _metric_axis_label(df: pd.DataFrame, metric: str, y_transform: str) -> str:
    unit = _metric_unit_label(df, metric)
